...
<body>
    ...
    <div class="content">
        ...
        <div class="media">
            ...
            <iframe src="https://youtu.be/embed/uCA8Q3nwaz4" frameborder="0" width="420" height="315"></iframe>
        </div>
        <!-- Documentation -->
        ...
        <!-- Query -->
        <div class="query">
            <form action="Gameobjects.php" method="post">
    ...
    if (isset($_GET["run"])) {
        ...
        $progress = 3; //the number of the progress where we get if we answer correctly (different in each page)
        ...
        if ($_GET["run"] == 1) {
            if ($db_progress == $progress - 1) {
                ...
                echo "<script>location.assign('http://" . $_SERVER['HTTP_HOST'] . "/NotDoneYet.php?" . str_replace("&run=1", "", $_SERVER['QUERY_STRING']) . "')</script>"; //Enter the next page here; we use JS here because PHP had problem with modifying header
            } else {
                echo "<script>alert('You have solved the task well, but you are more forward with the progress!')</script>";
                echo "<script>location.assign('http://" . $_SERVER['HTTP_HOST'] . "/Gameobjects.php?" . str_replace("&run=0", "", $_SERVER['QUERY_STRING']) . "')</script>"; //Enter here current page
            }
        }
    }
