...
<body>
    <?php
        ...
        function Progress($progress){
            ...
            $sql = "select `Progress` from `ugp` where `Username`='" . $_SESSION['uname'] . "' and `Password`='" . $_SESSION['passwd'] . "'";
            $db_progress = mysqli_query($mysqli, $sql);

            if ($db_progress == $progress - 1) {
                $sql = "update `ugp` set `Progress`='" . $progress . "' where `Username`='" . $_SESSION['uname'] . "' and `Password`='" . $_SESSION['passwd'] . "'";
    ...
    <script>
        ...
        for (i=0; i < pages.length; i++) {
            ...
            if (parseInt(pages[i].id.name) == currentProg) {
                page.classList.add("current");
            } else if (parseInt(pages[i].id.name) < currentProg) {
    ...
    <div class="sidebar">
        ...
        <a class="active pages" href="Create.php" name="0">Create main things</a>
        <a class="pages" href="Images.html" name="1">Import and use images</a>
        <a class="pages" href="Gameobjects.html" name="2">GameObjects introduced</a>
    </div>
